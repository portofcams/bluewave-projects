...
// LIVE "KING SALMON RIGHT NOW" CONDITIONS panel — the showpiece of the Alaska
// Rainbow Lodge sample site.
//
...
//     Weather Service public API for King Salmon Airport (ICAO PAKN):
//       https://api.weather.gov/stations/PAKN/observations/latest
...
//     honest behavior.
//   - Sunrise / sunset for King Salmon (58.68, -156.65) are COMPUTED
...
// floatplane-access lodge before booking, and a guest already there checking
// today's fly-out weather. It is explicitly NOT a substitute for the lodge's
// own guide team, who make the real go/no-go call each morning.
//
// NWS returns metric SI units in `json.properties`; we convert to familiar
// units (C/F, mph, miles, ft) below.
//
// All classes are Tailwind arbitrary values or the `.arl-*` scoped helpers
// from _shared.tsx. Nothing global is touched.

import { useEffect, useState } from "react";

const KING_SALMON_LAT = 58.68;
const KING_SALMON_LON = -156.65;
const ICAO = "PAKN"; // King Salmon Airport
...
// rendered in America/Anchorage (King Salmon's time zone), plus a daylight
...
// Realistic mid-July afternoon at King Salmon: light southwesterly wind, good
// visibility, scattered deck, comfortable temps for Bristol Bay. NEVER shown
// as live.
const SAMPLE: Decoded = {
  windText: "230° at 9 mph",
  visText: "10+ mi",
  skyText: "Scattered 4,500 ft",
  tempText: "16°C / 61°F",
  dewText: "9°C / 48°F",
  altimText: "29.88 inHg",
  flyRead: "Good",
  raw: "PAKN 031953Z 23008KT 10SM SCT045 16/09 A2988 RMK AO2",
  rawAssembled: false,
...
// is missing. Station is always PAKN; fields are space-joined in METAR order.
function composeMetar(p: NwsProps): string | null {
  const groups: string[] = ["PAKN"];
...
function decodeNws(p: NwsProps): Decoded {
  ...
  // ---- obs time (ISO -> local King Salmon / Anchorage time) ----
  let obsTime = "—";
  if (p.timestamp) {
    const dt = new Date(p.timestamp);
    if (!Number.isNaN(dt.getTime())) {
...
export function LodgeConditions() {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-[#f5f1e6]/12 bg-gradient-to-br from-[#132b22] via-[#1c3b2e] to-[#0c1a15] p-5 shadow-[0_24px_60px_-24px_rgba(0,0,0,0.7)] sm:p-6">
      ...
      <svg
        ...
        aria-hidden="true"
      >
        <path d="M-20 60 C 100 40, 180 80, 420 50" stroke="#a9c4b6" strokeWidth="1" fill="none" />
        <path d="M-20 120 C 100 100, 180 140, 420 110" stroke="#a9c4b6" strokeWidth="1" fill="none" />
        <path d="M-20 200 C 100 180, 180 220, 420 190" stroke="#a9c4b6" strokeWidth="1" fill="none" />
        <circle cx="330" cy="64" r="34" fill="none" stroke="#d79a3c" strokeWidth="0.8" />
      </svg>
      ...
      <div className="relative">
        ...
        <div className="mb-4 flex items-center justify-between gap-3">
          <div>
            <p className="arl-eyebrow !text-[#d79a3c]">King Salmon, right now</p>
            <h3 className="arl-display mt-1 text-xl font-semibold text-[#f5f1e6] sm:text-2xl">
              PAKN conditions
            </h3>
          </div>
          <span className="inline-flex items-center gap-1.5 rounded-full border border-[#4f7a63]/50 bg-[#0c1a15]/50 px-2.5 py-1 text-[10px] font-semibold uppercase tracking-[0.16em] text-[#a9c4b6]">
            <span className="relative flex h-1.5 w-1.5">
              <span
                className={`absolute inline-flex h-full w-full rounded-full ${
                  live ? "animate-ping bg-[#7fd0a0]" : "bg-[#a9c4b6]"
                } opacity-75`}
              />
              <span
                className={`relative inline-flex h-1.5 w-1.5 rounded-full ${
                  live ? "bg-[#7fd0a0]" : "bg-[#a9c4b6]"
                }`}
              />
            </span>
            {d.flyRead !== "—" ? `${d.flyRead} flying` : "PAKN"}
          </span>
        </div>
        ...
        <div className="mt-4 arl-glass p-3">
          <div className="mb-2 flex items-center justify-between">
            <span className="text-[11px] font-semibold uppercase tracking-[0.16em] text-[#a9c4b6]">
              Raw observation
            </span>
            <SourceBadge source={s.source} />
          </div>
          <code className="block overflow-x-auto whitespace-nowrap font-mono text-[12px] leading-relaxed text-[#dce8db]">
            {d.raw}
          </code>
          {live && d.rawAssembled && d.raw !== "—" && (
            <p className="mt-1.5 text-[10px] italic text-[#dce8db]/45">
        ...
        {/* honest footnote */}
        <p className="mt-3 text-[11px] leading-relaxed text-[#dce8db]/60">
          Pulls the live observation for{" "}
          <span className="font-semibold text-[#f5f1e6]/85">King Salmon Airport (PAKN)</span>{" "}
          from the National Weather Service API (no key, CORS-enabled) — the
          nearest station to the lodge. Sunrise/sunset/daylight are computed
          for King Salmon (58.68°N, 156.65°W). The &quot;flying today&quot;
          read is a friendly heuristic for trip planning — the lodge&apos;s
          own pilots and guides make the real daily fly-out call, in the air
          and on the water. If the feed is unreachable, this panel shows a
          clearly-labeled <span className="font-semibold text-[#f5f1e6]/85">sample</span>{" "}
...
function Metric({
  ...
  return (
    <div className="arl-glass flex flex-col p-3">
      <div className="mb-1.5 flex items-center justify-between">
        <span className="text-[#a9c4b6]">
      ...
      <span className="arl-display text-[15px] font-semibold leading-tight text-[#f5f1e6] sm:text-base">
        {value}
      </span>
      <span className="mt-1 text-[10px] uppercase tracking-[0.12em] text-[#dce8db]/45">
        {label}
      </span>
      <span className="mt-0.5 text-[11px] text-[#dce8db]/60">{sub}</span>
    </div>
  );
}
...
function SourceBadge({ source }: { source: Source }) {
  if (source === "loading")
    return (
      <span className="rounded-full border border-[#f5f1e6]/20 px-1.5 py-0.5 text-[8px] font-semibold uppercase tracking-[0.12em] text-[#f5f1e6]/45">
        …
      </span>
    );
  const map: Record<Exclude<Source, "loading">, { t: string; cls: string }> = {
    live: {
      t: "Live · NWS",
      cls: "border-[#7fd0a0]/50 bg-[#7fd0a0]/15 text-[#a9e6c0]",
    },
    computed: {
      t: "Computed",
      cls: "border-[#d79a3c]/45 bg-[#d79a3c]/12 text-[#e8bd77]",
    },
    sample: {
      t: "Sample",
      cls: "border-[#f5f1e6]/25 bg-[#f5f1e6]/8 text-[#f5f1e6]/70",
    },
  };
  const m = map[source];
  return (
